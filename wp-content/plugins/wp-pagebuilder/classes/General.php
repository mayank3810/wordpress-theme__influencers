	...
	class WPPB_General{
		...
		public function posts_table_row_actions( $actions, $post ) {
			...
			if ( in_array( $post->post_type, $wppb_supports_post_types ) ) {
				...
				$actions['wppb_editor'] = '<a href="'.$editor_url.'">'.__('Edit with Distinct Cloud', 'wp-pagebuilder' ).'</a>';
			}
			return $actions;
		}
			...
			<?php if( $editor == 'wppb_builder_activated' ) { ?>
				<div class="wppb-edit-button-wrap">
					...
					<div class="wppb-editor-warper">
						<a href="<?php echo wppb_helper()->get_editor_url($post->ID); ?>" class="wppb-backend-btn wppb-primary-btn edit-with-wppb-builder"><?php _e('Edit With Distinct Cloud','wp-pagebuilder');?></a>
					</div>
